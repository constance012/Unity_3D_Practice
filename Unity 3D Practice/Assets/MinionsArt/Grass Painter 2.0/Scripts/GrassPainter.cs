...
/// <summary>
/// Grass Painter tool by Minions Art.
/// </summary>
...
public class GrassPainter : MonoBehaviour
{
	...
	public void GenerateAtRuntime(Ray ray, float maxHeight)
	{
		...
		for (int k = 0; k < density * brushSize; k++)
		{
			...
			if (Physics.Raycast(ray, out RaycastHit terrainHit, maxHeight, hitMask.value) && i < grassLimit && terrainHit.normal.y <= (1 + normalLimit) && terrainHit.normal.y >= (1 - normalLimit))
			{
